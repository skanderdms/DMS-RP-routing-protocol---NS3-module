...
*/
              ...
              if (lcb.IsNull () == false)
              ...
              else
                {
                  ...
                  NS_LOG_UNCOND ("Unable to deliver packet locally due to null callback " << p->GetUid () << " from " << origin);
              ...
              if (header.GetTtl () > 1)
                {
                  ...
                  NS_LOG_UNCOND ("Forward broadcast. TTL " << (uint16_t) header.GetTtl ());
                  ...
                  if (m_routingTableDown.LookupRoute (dst, toBroadcast))
                  ...
                  else
                    {
                      ...
                      NS_LOG_UNCOND ("No route to forward broadcast. Drop packet " << p->GetUid ());
                    }
                }
              else
                {
 ...
 //NS_LOG_UNCOND("NODE 16 received iot packet: "<<Simulator::Now ());
  ...
  if (m_socketAddresses.find (socket) != m_socketAddresses.end ())
  ...
  else if (m_socketSubnetBroadcastAddresses.find (socket) != m_socketSubnetBroadcastAddresses.end ())
    {
     ...
     // NS_LOG_UNCOND ("BROAAAAAAAAAAAAAAAAAAAAAAAAAADECAST");
    }
  else
    {
   ...
   //       uint32_t id = advertiseHeader.GetId ();  //DMS
      ...
      //       NS_LOG_UNCOND(receiver<<" Ignoring ADVERTISE: hops> maxhops");
       ...
       if(modifyRoutingTable)  /// modify the route
        ...
        // NS_LOG_UNCOND(receiver<<" Route added to TABLE 3"); //DMS To delete
        }
...
NS_LOG_UNCOND("Address of output device is: " <<m_sinkOutPutDevice->GetAddress());
  ...
  //  NS_LOG_UNCOND(" :is a SINK  : "<<(j->second).GetLocal ());
        rt.SetSource(j->second.GetLocal ());    NS_LOG_UNCOND("the inetrface for device is: " <<m_ipv4->GetInterfaceForDevice (m_sinkOutPutDevice));
