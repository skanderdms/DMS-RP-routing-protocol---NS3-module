 ...
 * \ingroup dmsrp
 *
 * \brief DMSRP loopback UDP echo test case
 */
...
LoopbackTestCase::DoRun ()
{
  ...
  // Setup TCP/IP & DMSRP
  DmsrpHelper dmsrp; // Use default parameters here
  InternetStackHelper internetStack;
  internetStack.SetRoutingHelper (dmsrp);
 ...
 * \ingroup tests
 *
 * \brief DMSRP Loopback test suite
 */
class DmsrpLoopbackTestSuite : public TestSuite
{
public:
  DmsrpLoopbackTestSuite () : TestSuite ("routing-dmsrp-loopback", SYSTEM)
  {
    SetDataDir (NS_TEST_SOURCEDIR);
    // UDP Echo loopback test case
    AddTestCase (new LoopbackTestCase (), TestCase::QUICK);
  }
} g_dmsrpLoopbackTestSuite; ///< the test suite
