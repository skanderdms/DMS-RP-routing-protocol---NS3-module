...
double m_interval = 0.2;                          // Interval between two transmited packets (sencends)
...
//EnergySourceContainer source ;
 sources = basicSourceHelper.Install (wifiNodes); NS_LOG_UNCOND("step 3");

/*
for(uint32_t i=0;i<m_nWifiNodes;i++)
 {
                rand=m_uniformRandomVariable->GetValue(8,12);
 ...
 ///////// basicSourceHelper.Set ("PeriodicEnergyUpdateInterval",TimeValue (Seconds (updateIntervalS)));
  // install source
   source = basicSourceHelper.Install (wifiNodes.Get(i)); NS_LOG_UNCOND("step 3");
   sources.Add(source);
 }
*/
/*
...
if(m_protocol==2)
{
 ...
 serverAddress =SrvInterfacesContainer.GetAddress(0);//
